...
(function () {
    ...
    var sendMessageUrl = getSetting("sendMessageUrl");
    ...
    var plugin = {
        postSubmitted: function (data, callback) {
            ...
            Topics.getTopicData(data.tid, function (err, topic) {
                ...
                Topics.getTopicTags(data.tid, function (err, tags) {
                    ...
                    if (!tags || !tags.length) return callback();
                    ...
                    User.getUserData(data.uid, function (err, user) {
                        ...
                        var post = {
                            ...
                            requiresReview: !hasNotifyOnPost(tags)
                        };
                        console.log("Sending post ", post, " to url ", sendPostUrl);
                        request
                            .post(sendPostUrl)
                            .send(post)
        ...
        notificationPushed: function (data, callback) {
            ...
            async.parallel([
                ...
                console.log("Sending notification ", body, " to url ", sendNotificationUrl);
                request
        ...
        messageSent: function (data, callback) {
            callback = callback || function (err) {};

            User.getUserData(data.touid, function (err, toUser) {
                if (err) {
                    console.log("Error: ", err);
                    return callback(null, data);
                }
                if (!toUser) {
                    console.log("No to user found");
                    return callback(null, data);
                }

                User.getUserData(data.fromuid, function (err, fromUser) {
                    if (err) {
                        console.log("Error: ", err);
                        return callback(null, data);
                    }
                    if (!fromUser) {
                        console.log("No from user found");
                        return callback(null, data);
                    }
                    ...
                    console.log("Sending message ", message, " to url ", sendMessageUrl);
                    request
                        .post(sendMessageUrl)
                        .send(message)
                        .auth(username, password)
